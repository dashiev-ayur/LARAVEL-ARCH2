...
## Текущий контекст пользователя (команда и организация)

- Текущая команда хранится в `users.current_team_id`.
- Текущая организация хранится в `users.current_org_id`.
- `current_org_id` синхронизируется со сменой команды: после `switchTeam` выбирается первая доступная организация новой команды (по алфавиту) или `null`, если организаций нет.
- В `users.current_org_id` настроен `foreign key` с `nullOnDelete`, чтобы при удалении организации ссылка корректно очищалась.
