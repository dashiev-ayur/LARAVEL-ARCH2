...
- Для таблицы используется `@tanstack/react-table` и компоненты из `resources/js/components/table`.
- Текущая версия работает без лимита по количеству и без сортировок.
